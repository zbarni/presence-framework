...
//@zajzi
PJ_DEF(pj_str_t) pjsua_get_gruu(pjsua_acc_id acc_id) {
	PJ_LOG(4,(THIS_FILE, "@zreg PJSIP GRUU is %.*s",pjsua_var.acc[acc_id].gruu.slen, pjsua_var.acc[acc_id].gruu.ptr));
	return pjsua_var.acc[acc_id].gruu;
}
...
PJ_DEF(pj_status_t) pjsua_acc_set_online_status( pjsua_acc_id acc_id,
	...
	PJ_LOG(4,(THIS_FILE, "Acc %d: @zajzi1 setting online status to %d..",
			acc_id, is_online));
	pj_log_push_indent();
...
//	PJ_LOG(4,(THIS_FILE, "@zreg NAT check is called"));
...
//			PJ_LOG(4,(THIS_FILE, "@zreg CONTACT FROM RESPONSE. NAME is %.*s   ;###; VALUE is : %.*s ",contact_hdr->other_param.name.slen,contact_hdr->other_param.name.ptr,contact_hdr->other_param.value.slen, contact_hdr->other_param.name.ptr));
			pj_assert(uri != NULL);
...
{
//	PJ_LOG(4, (THIS_FILE, "@zreg regc_tsx_cb called, message is %.*s",param->cbparam.rdata->msg_info.len,param->cbparam.rdata->msg_info.msg_buf));
...
static void extract_gruu (pjsip_rx_data *rdata) {
//	pool = pjsua_pool_create("tmp", 512, 512);
//				contact_hdr = (pjsip_contact_hdr*)
//					  pjsip_parse_hdr(pool, &STR_CONTACT, acc->contact.ptr,
//							  acc->contact.slen, NULL);
...
//	PJ_LOG(4, (THIS_FILE, "@zreg regc_cb was called, contact at this moment is : %.*s",acc->contact.slen, acc->contact.ptr));
...
//			PJ_LOG(4,(THIS_FILE, "@zreg before calling NAT is %.*s",acc->contact.slen, acc->contact.ptr));
...
//			PJ_LOG(4,(THIS_FILE, "@zreg after calling NAT %.*s",acc->contact.slen, acc->contact.ptr));
...
//			extract_gruu(param->rdata);
			acc->gruu = acc->contact;
...
//		PJ_LOG(4, (THIS_FILE, "@zreg on_reg_state2 will be called"));
...
//	PJ_LOG(4, (THIS_FILE, "@zreg boo contact is : %.*s", acc->contact.slen, acc->contact.ptr));
